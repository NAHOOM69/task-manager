...
const TaskManager: React.FC = () => {
  ...
  useEffect(() => {
    const saveSuccess = StorageManager.saveTasks(tasks);
    if (!saveSuccess) {
      alert('שגיאה בשמירת הנתונים. אנא רענן את הדף ונסה שוב.');
    }
  }, [tasks]);
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col">
              <label className="text-sm text-gray-600 mb-1">תאריך תזכורת</label>
              <input
                type="datetime-local"
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">משימה</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredAndSearchedTasks.map((task) => (
                <tr key={task.id} className={task.completed ? "bg-gray-50" : "bg-white"}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{task.clientName}</td>
